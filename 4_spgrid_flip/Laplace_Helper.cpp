...
struct Laplace_Thread_Helper : public PTHREAD_QUEUE::TASK {
  ...
  Laplace_Thread_Helper(Laplace_Helper<T, log2_struct, d> *const obj_input,
                        const int index_start_input,
                        const int index_end_input)
...
void Laplace_Helper<T, log2_struct, 2>::Run_Parallel(const int number_of_partitions) {
  for (int partition = 0; partition < number_of_partitions; partition++) {
    // Calculate indicies of current partition
    int first_index_of_partition =
        (size / number_of_partitions) * (partition) + std::min(size % number_of_partitions, partition);
    int last_index_of_partition =
        (size / number_of_partitions) * (partition + 1) + std::min(size % number_of_partitions, partition + 1) - 1;
    // Create helper object
    Laplace_Thread_Helper<T, log2_struct, d>
        *task = new Laplace_Thread_Helper<T, log2_struct, d>(this, first_index_of_partition, last_index_of_partition);
    // Enqueue
...
template<class T, int log2_struct>
void Laplace_Helper<T, log2_struct, 2>::Run_Index_Range(const int index_start, const int index_end) {
      ...
      for (int j = ymin; j <= ymax; j++) {
        ...
        if (mask_value & (SPGrid_Cell_Type_Interior | SPGrid_Cell_Type_Ghost)) {
          ...
          if (mask_value & SPGrid_Face_Minus_X_Scaled)
            result += scale_nonuniform
                * ((double) (*reinterpret_cast<T *>(y_base_addr + o_grid[i - 1][j])) - y1_in[cur_index]);
          else if (mask_value & SPGrid_Face_Minus_X_Active)
            result +=
                scale_uniform * ((double) (*reinterpret_cast<T *>(y_base_addr + o_grid[i - 1][j])) - y1_in[cur_index]);
          ...
          if (mask_value & SPGrid_Face_Minus_Y_Scaled)
            result += scale_nonuniform
                * ((double) (*reinterpret_cast<T *>(y_base_addr + o_grid[i][j - 1])) - y1_in[cur_index]);
          else if (mask_value & SPGrid_Face_Minus_Y_Active)
            result +=
                scale_uniform * ((double) (*reinterpret_cast<T *>(y_base_addr + o_grid[i][j - 1])) - y1_in[cur_index]);
...
//#####################################################################
template<class T, int log2_struct>
void Laplace_Helper<T, log2_struct, 3>::Run_Parallel(const int number_of_partitions) {
  for (int partition = 0; partition < number_of_partitions; partition++) {
    // Calculate indicies of current partition
    int first_index_of_partition =
        (size / number_of_partitions) * (partition) + std::min(size % number_of_partitions, partition);
  ...
  for (int i = xmin; i <= xmax; i++)
    for (int j = ymin; j <= ymax; j++)
      for (int k = zmin; k <= zmax; k++) {
        o_grid[i][j][k] =
            packed_offset + simple_offset;  // Can do simple addition here since addresses are within block
        simple_offset += sizeof(T);
      }
...
// T_MASK corresponds to the mask for the data (not the mask channel)
template<class T, int log2_struct>
void Laplace_Helper<T, log2_struct, 3>::Run_Index_Range(const int index_start, const int index_end) {
  int count = 0;
  // Compute shadow grid of linear offsets
  unsigned long
      *offset_grid_ptr = (unsigned long *) malloc((og_xsize) * (og_ysize) * (og_zsize) * sizeof(unsigned long));
  typedef unsigned long (&offset_grid_type)[og_xsize][og_ysize][og_zsize];
  offset_grid_type o_grid = reinterpret_cast<offset_grid_type>(*offset_grid_ptr);
      ...
      for (int j = ymin; j <= ymax; j++)
        for (int k = zmin; k <= zmax; k++) {
          ...
          if (mask_value & (SPGrid_Cell_Type_Interior | SPGrid_Cell_Type_Ghost)) {
            ...
            if (mask_value & SPGrid_Face_Minus_X_Scaled)
              result += scale_nonuniform
                  * ((double) (*reinterpret_cast<T *>(y_base_addr + o_grid[i - 1][j][k])) - y1_in[cur_index]);
            else if (mask_value & SPGrid_Face_Minus_X_Active)
              result += scale_uniform
                  * ((double) (*reinterpret_cast<T *>(y_base_addr + o_grid[i - 1][j][k])) - y1_in[cur_index]);
            ...
            if (mask_value & SPGrid_Face_Minus_Y_Scaled)
              result += scale_nonuniform
                  * ((double) (*reinterpret_cast<T *>(y_base_addr + o_grid[i][j - 1][k])) - y1_in[cur_index]);
            else if (mask_value & SPGrid_Face_Minus_Y_Active)
              result += scale_uniform
                  * ((double) (*reinterpret_cast<T *>(y_base_addr + o_grid[i][j - 1][k])) - y1_in[cur_index]);
            ...
            if (mask_value & SPGrid_Face_Minus_Z_Scaled)
              result += scale_nonuniform
                  * ((double) (*reinterpret_cast<T *>(y_base_addr + o_grid[i][j][k - 1])) - y1_in[cur_index]);
