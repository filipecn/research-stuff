...
public:
  explicit Laplace_Helper(T *const x_input,
                          const T *const y_input,
                          const unsigned *const mask_input,
                          const unsigned long *const b_input,
                          const int size_input,
                          const double scale_uniform_in,
                          const double scale_nonuniform_in)
      : x(x_input), y(y_input), mask(mask_input), b(b_input), size(size_input), scale_uniform(scale_uniform_in),
        scale_nonuniform(scale_nonuniform_in) {}
  void Run() { Run_Index_Range(0, size - 1); }
