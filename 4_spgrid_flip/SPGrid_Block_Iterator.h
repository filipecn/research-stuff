...
public:
  ...
  inline bool Valid() const { return block_index < (int) size; }

  inline void Next() {
    element_index = (element_index + 1) & element_index_mask;
    if (!element_index)
      block_index++;
  }
  ...
  typename T_ARRAY::DATA &Data(T_ARRAY &array, unsigned long offset_input) const {
    typedef typename T_ARRAY::DATA T;
    unsigned long
        offset = T_ARRAY::MASK::Packed_Add(block_offsets[block_index] + element_index * sizeof(T), offset_input);
...
public:
  ...
  SPGrid_Reverse_Block_Iterator(const unsigned long *const block_offsets_input, const unsigned size_input)
      : block_offsets(block_offsets_input), size(size_input), block_index(size - 1),
        element_index(elements_per_block - 1) {}

  SPGrid_Reverse_Block_Iterator(const std::pair<const unsigned long *, unsigned> &blocks)
      : block_offsets(blocks.first), size(blocks.second), block_index(size - 1),
        element_index(elements_per_block - 1) {}

  inline bool Valid() const { return block_index >= 0; }

  inline void Next() {
    if (!element_index)
      block_index--;
    element_index = (element_index + element_index_mask) & element_index_mask;
  }
