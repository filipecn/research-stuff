...
template <class T_SET> struct ADAPTIVE_SPHERE_RASTERIZER {
...
public:
  ADAPTIVE_SPHERE_RASTERIZER(T_SET &set_input, std_array<float, 3> center_input,
                             float inner_radius_input, float outer_radius_input)
      : set(set_input), center(center_input), inner_radius(inner_radius_input),
        outer_radius(outer_radius_input),
        inner_radius_squared(inner_radius * inner_radius),
        outer_radius_squared(outer_radius * outer_radius), total_active(0) {}
  ...
  static std_array<float, 3> Clamp(const std_array<float, 3> v,
                                   const std_array<float, 3> vmin,
                                   const std_array<float, 3> vmax) {
    return std_array<float, 3>(Clamp(v.data[0], vmin.data[0], vmax.data[0]),
                               Clamp(v.data[1], vmin.data[1], vmax.data[1]),
                               Clamp(v.data[2], vmin.data[2], vmax.data[2]));
  }

  float Maximum_Distance_Squared(const std_array<float, 3> Xmin,
                                 const std_array<float, 3> Xmax) {
    return Maximum_Distance_Squared(Xmin.data[0], Xmax.data[0], Xmin.data[1],
                                    Xmax.data[1], Xmin.data[2], Xmax.data[2]);
  }

  float Maximum_Distance_Squared(float Xmin, float Xmax, float Ymin, float Ymax,
  ...
  float Minimum_Distance_Squared(const std_array<float, 3> Xmin,
                                 const std_array<float, 3> Xmax) {
  ...
  bool Consume(GEOMETRY_BLOCK block) {
    ...
    if (min_dist_sq > outer_radius_squared)
      return false; // outside band, don't recurse

    float max_dist_sq = Maximum_Distance_Squared(block.Xmin, block.Xmax);
    if (max_dist_sq < inner_radius_squared)
      return false; // inside band, don't recurse
