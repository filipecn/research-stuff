...
public:
  ...
  uint64_t max_linear_offset; // TODO: Change semantics to make this the first
                              // offset that is *invalid*
  std::vector<uint64_t> block_offsets;
  bool dirty; // Indicates that block offsets are inconsistent with the bitmap
              // (perhaps for a good reason, if only one of them is used)

  // Single size argument constructor
  SPGrid_Set(T_ARRAY array_input) : array(array_input) {
    ...
    page_mask_array =
        new uint64_t[array_length]; // TODO: Why "new" and not Raw_Allocate() ?
    memset(reinterpret_cast<void *>(page_mask_array), 0,
           array_length * sizeof(uint64_t)); // TODO: Is this really needed?
    max_linear_offset =
        array.geometry.Padded_Volume() *
        T_ARRAY::MASK::Bytes_Per_Element(); // TODO: Check this is correct
    pthread_mutex_init(
        &pm_mutex, 0); // TODO: Check to see where these mutexes are really used
    dirty = false;     // They start as consistent -- both empty
  }

  ~SPGrid_Set() { delete page_mask_array; }
  ...
  void MarkPageActive(uint64_t linear_offset) {
    if (linear_offset < max_linear_offset) {
      ...
      FATAL_ERROR("Linear offset " + Value_To_String(linear_offset) +
                  " is out of range (upper limit = " +
                  Value_To_String(max_linear_offset) + ")");
  }
  ...
  void Mask(const std_array<unsigned int, dim> coord, const T mask) {
    T &data = array(coord);
    uint64_t linear_offset = reinterpret_cast<uint64_t>(&data) -
                             reinterpret_cast<uint64_t>(array.Get_Data_Ptr());
    uint64_t page_mask = 1UL << (linear_offset >> 12 & 0x3f);
  ...
  void Mask(uint64_t linear_offset, const T mask) {
    if (linear_offset < max_linear_offset) {
      T *data = reinterpret_cast<T *>(
          reinterpret_cast<uint64_t>(array.Get_Data_Ptr()) + linear_offset);
      uint64_t page_mask = 1UL << (linear_offset >> 12 & 0x3f);
      page_mask_array[linear_offset >> 18] |= page_mask;
      dirty = true;
      *data |= mask;
  ...
  bool Is_Set(uint64_t linear_offset, const T mask) const {
    if (linear_offset < max_linear_offset) {
      if (page_mask_array[linear_offset >> 18] &
          (1UL << (linear_offset >> 12 & 0x3f))) {
        T *data = reinterpret_cast<T *>(
            reinterpret_cast<uint64_t>(array.Get_Data_Ptr()) + linear_offset);
        return (*data & mask);
      }
  ...
  void FillBitmapWithBlockOffsets(std::vector<uint64_t> block_offsets) {
    dirty = true;
    for (std::vector<uint64_t>::iterator it = block_offsets.begin();
